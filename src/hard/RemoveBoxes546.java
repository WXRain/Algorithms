...
public class RemoveBoxes546 {
    ...
    private int calculatePoint(int[] boxes, int[][][] dp, int l, int r, int k) {
        if (l > r) return 0;
        ...
        for (int i = l; i < r; i++) {
            if (boxes[i] == boxes[r]) {
                dp[l][r][k] = Math.max(dp[l][r][k], calculatePoint(boxes, dp, l, i, k + 1) +
                        calculatePoint(boxes, dp, i + 1, r - 1, 0));
            }
        }
        return dp[l][r][k];
    }
